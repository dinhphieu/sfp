    ...
    public async getScratchOrgsByTag(tag: string, isMyPool: boolean, unAssigned: boolean) {
        ...
        return retry(
            async (bail) => {
                ...
                if (tag)
                    query = `SELECT Pooltag__c, Id, CreatedDate, ScratchOrg, ExpirationDate, SignupUsername, SignupEmail, Password__c, Allocation_status__c, LoginUrl, SfdxAuthUrl__c, PullRequestId__c FROM ScratchOrgInfo WHERE Pooltag__c = '${tag}' AND Status = 'Active' `;
                else
                    query = `SELECT Pooltag__c, Id, CreatedDate, ScratchOrg, ExpirationDate, SignupUsername, SignupEmail, Password__c, Allocation_status__c, LoginUrl, SfdxAuthUrl__c, PullRequestId__c FROM ScratchOrgInfo WHERE Pooltag__c != null AND Status = 'Active' `;
